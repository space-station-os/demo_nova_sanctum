...
void ElectrolysisNode::handle_electrolysis_request(
    ...
    water_available_ = true;  // Mark electrolysis as ready
...
void ElectrolysisNode::performElectrolysis() {
    if (!water_available_ || water_level_ <= 0) {
        ...
        water_available_ = false;  // Ensure new water can be accepted
        return;
    }
    ...
    double energy_required_per_mol = 237.1;  // kJ/mol (Faraday’s laws of electrolysis)
    double water_moles = water_level_ * 55.5; // Convert liters to moles (55.5 mol/L)
    ...
    // Publish the electrolysis gas message
    ...
    gas_msg.temperature.temperature = temperature_ + (actual_energy_used / 1000);  // Approximate heating effect
    gas_msg.pressure.fluid_pressure = pressure_ + (hydrogen_moles * 0.1); // Approximate pressure increase
    ...
    if (water_level_ <= 0) {
        water_available_ = false;  // Mark electrolysis as available for new water
